...
LOCALPROC DoEmulateOneTick(void)
{
	...
	if ((ui3b) -1 == SpeedValue) {
        ...
        if (SpeedValue == 0) {
            ...
            // I've added a 1.3x speed setting in order to more
            // closely match the speed of a 68030.
            ExtraAdd = 9;
        } else {
            ExtraAdd = (kNumSubTicks << (SpeedValue-1)) - kNumSubTicks;
        }
        
        ExtraLimit = ExtraAdd << 3;
        
        //printf("SpeedValue: %d  ExtraAdd: %d\n", SpeedValue, ExtraAdd);
