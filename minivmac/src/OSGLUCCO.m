...
// Bri change!
long postBootSpeed = WantInitSpeedValue;
...
GLOBALOSGLUFUNC tMacErr HTCEexport(tPbuf i)
{
...

	{
        ...
        if ([ss hasPrefix:@"MSG:"]) {
            static NSTimer *setRunInBackgroundTimer = nil;
            
            NSString *msg = [ss substringFromIndex:4];
            //fprintf(stderr, "Message! %s\n", [msg UTF8String]);
            
            if ([msg isEqualToString:@"boot"]) {
                RunInBackground = false;
                [[NSFileManager defaultManager] createFileAtPath:[MyDataPath stringByAppendingPathComponent:@"signals/booted.txt"]
                                                        contents:[NSData data]
                                                      attributes:nil];
                
            } else if ([msg isEqualToString:@"start"]) {
                SpeedValue = postBootSpeed;
                if (setRunInBackgroundTimer) {
                    //NSLog(@"... canceled run in background timer");
                    [setRunInBackgroundTimer invalidate];
                    setRunInBackgroundTimer = nil;
                }
                RunInBackground = true;
                
            } else if ([msg isEqualToString:@"quit"]) {
                SpeedValue = 6;
                setRunInBackgroundTimer = [NSTimer scheduledTimerWithTimeInterval:4.0 repeats:NO block:^(NSTimer *timer) {
                    //NSLog(@"...resetting run in background");
                    setRunInBackgroundTimer = nil;
                    RunInBackground = false;
                }];
                
                [[NSFileManager defaultManager] createFileAtPath:[MyDataPath stringByAppendingPathComponent:@"signals/closed.txt"]
                                                        contents:[NSData data]
                                                      attributes:nil];
            }
			
		} else if ([ss hasPrefix:@"OUT:"]) {
			printf("%s", [[ss substringWithRange:NSMakeRange(4, [ss length]-4)] UTF8String]);
...
int main(int argc, char **argv)
{
	...
	if (argc >= 2) {
		char *end;
		postBootSpeed = strtol(argv[1], &end, 10);
		printf("Speed set to: %ld\n", postBootSpeed);
	} else {
		postBootSpeed = WantInitSpeedValue;
	}
